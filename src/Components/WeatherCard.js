...
const WeatherCard = ({ city }) => {
  ...
  if (loading || !weatherData) {
    return (
      <h1 className="text-center mt-40 text-xl text-gray-500 dark:text-gray-300">
        Loading weather...
      </h1>
    );
  }
  ...
  return (
    <div className="max-w-sm sm:max-w-md md:max-w-lg w-full mx-auto mt-10 bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 text-center transition-colors duration-300">
      {/* ICON */}
      <div className="flex justify-center -mt-16">
        <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full bg-white dark:bg-gray-700 shadow-lg flex items-center justify-center">
          <img
            src={`https://openweathermap.org/img/wn/${weatherData.weather[0].icon}@2x.png`}
            alt="weather"
      ...
      {/* TEMP */}
      <p className="text-4xl sm:text-5xl font-extrabold text-blue-600 dark:text-blue-400 mt-2">
        {(weatherData.main.temp - 273.15).toFixed(1)}°C
      </p>

      {/* CONDITION */}
      <p className="text-gray-500 dark:text-gray-300 mt-1 capitalize">
        {weatherData.weather[0].description}
      </p>
      ...
      <div className="grid grid-cols-2 gap-4 mt-6 text-sm sm:text-base">
        <div>
          <p className="text-gray-400 dark:text-white text-lg sm:text-xl">Feels Like</p>
          <p className="font-semibold dark:text-white text-lg sm:text-xl">
            {(weatherData.main.feels_like - 273.15).toFixed(1)}°C
          </p>
        </div>

        <div>
          <p className="text-gray-400 dark:text-white text-lg sm:text-xl">Humidity</p>
          <p className="font-semibold dark:text-white text-lg sm:text-xl">
            {weatherData.main.humidity}%
          </p>
        </div>
        ...
        <div>
          <p className="text-gray-400 dark:text-white text-lg sm:text-xl">Wind</p>
          <p className="font-semibold dark:text-white text-lg sm:text-xl">
            {(weatherData.wind.speed * 3.6).toFixed(1)} km/h
          </p>
        </div>
        ...
        <div>
          <p className="text-gray-400 dark:text-white text-lg sm:text-xl">Pressure</p>
          <p className="font-semibold dark:text-white text-lg sm:text-xl">
            {weatherData.main.pressure} hPa
          </p>
        </div>
      ...
      {/* SUN */}
      <div className="flex justify-between mt-6 text-gray-500 dark:text-white text-sm sm:text-base">
        <span>
          🌅 {new Date(weatherData.sys.sunrise * 1000).toLocaleTimeString()}
        </span>
        <span>
